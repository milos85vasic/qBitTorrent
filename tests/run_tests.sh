...
test_container_operations() {
    ...
    test_start "Check if port 7186 is listening"
    if assert_container_running "qbittorrent"; then
        if assert_port_listening "7186"; then
            test_pass "Port 7186 is listening"
        else
            test_fail "Port 7186 is not listening"
        fi
    else
    ...
    if assert_container_running "qbittorrent"; then
        if command -v curl &> /dev/null; then
            local http_code
            http_code=$(curl -s -o /dev/null -w "%{http_code}" --connect-timeout 5 "http://localhost:7186" 2>/dev/null) || http_code="000"
