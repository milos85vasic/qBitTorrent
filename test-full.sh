...
curl -s --max-time 5 "http://localhost:8085/" | grep -q "qBittorrent" && echo "   ✓ Login page loads"

# Test login
echo ""
echo "3. Testing login..."
rm -f /tmp/test_cookie.txt
if curl -s --max-time 5 -c /tmp/test_cookie.txt -X POST "http://localhost:8085/api/v2/auth/login" \
...
SEARCH_ID=$(curl -s --max-time 5 -b /tmp/test_cookie.txt -X POST "http://localhost:8085/api/v2/search/start" \
...
if [ -n "$SEARCH_ID" ]; then
    ...
    # Get results
    RESULTS=$(curl -s --max-time 5 -b /tmp/test_cookie.txt "http://localhost:8085/api/v2/search/results?id=$SEARCH_ID&limit=1")
    if echo "$RESULTS" | grep -q '"fileName"'; then
        ...
        if curl -s --max-time 10 -b /tmp/test_cookie.txt -X POST "http://localhost:8085/api/v2/torrents/add" \
            -d "urls=$URL" | grep -q "Ok"; then
            echo "   ✓ Download successful"
            
            sleep 2
            # Verify
            if curl -s --max-time 5 -b /tmp/test_cookie.txt "http://localhost:8085/api/v2/torrents/info" | grep -q '"name":'; then
